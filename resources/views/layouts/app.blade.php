...
<div class="app-shell">
    <aside class="sidebar" id="sidebar">
        <a class="brand" href="{{ route('dashboard') }}"><span class="brand-mark">7</span><span><strong>INOVA7</strong><small>Gestão Acadêmica</small></span></a>
        <nav class="nav flex-column gap-1 mt-4">
            <a class="nav-link {{ request()->routeIs('dashboard') ? 'active' : '' }}" href="{{ route('dashboard') }}">▦ Painel</a>
            <a class="nav-link {{ request()->routeIs('planning.*') ? 'active' : '' }}" href="{{ route('planning.index') }}">▤ Planejamento</a>
            <a class="nav-link" href="#">◫ Cursos e matrizes</a>
            <a class="nav-link" href="#">♙ Professores</a>
            <a class="nav-link {{ request()->routeIs('audit.*') ? 'active' : '' }}" href="{{ route('audit.index') }}">✓ Auditoria</a>
            <a class="nav-link" href="#">⇧ Importações</a>
        </nav>
        <div class="sidebar-footer"><span class="avatar">LT</span><span><strong>Larissa Torres</strong><small>Coordenação</small></span></div>
    </aside>
    <main class="content">
        <header class="topbar">
